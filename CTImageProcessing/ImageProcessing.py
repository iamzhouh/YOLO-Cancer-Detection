...
with open('label.csv', 'w', encoding='utf8', newline='') as f:
    writer = csv.writer(f)
    writer.writerow(['number', 'x_start', 'y_start', 'x_end', 'y_end', 'img_path'])
...
def read_dicom_and_nii_to_label(nii_file_path,img_file_path):    # 传入 标注路径 和 单个病人的图像目录
    ...
    for file_name in file_name_list:
        if file_name != "Untitled.nii":
            file_path = os.path.join(img_file_path,file_name)
            ds = pydicom.dcmread(file_path)

            if (ds.SeriesDescription == "Venous Phase  5.0  B30f") | (ds.SeriesDescription == "Venous Phase  7.0  B30f") | (ds.SeriesDescription == "") | (ds.SeriesDescription == "Venous Phase  5.0  B20f") | (ds.SeriesDescription == "Venous Phase  7.0  B20f"):
                j += 1
...
if __name__ == "__main__":
    folder_path = r"D:/Download/MedicalDataSet/dataset"
    patient_list = os.listdir(folder_path)   # 得到多个病人的目录列表
    for patient_list_name in patient_list:
        img_path = os.path.join(folder_path, patient_list_name) + '/S0000000'    # 取得单个病人的图像目录
        nii_file_path = os.path.join(folder_path, patient_list_name) + '/Untitled.nii'   # 取得标注路径
