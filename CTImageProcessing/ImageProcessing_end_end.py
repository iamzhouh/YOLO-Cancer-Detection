        ...
        if len(file_name.split('.')) == 1:
            ...
            if ((ds.SeriesDescription == "Venous Phase  5.0  B30f") | (ds.SeriesDescription == "Venous Phase  7.0  B30f") \
                ...
                if img_number-j in mark_index_and_coordinates:
                    RelativePath = os.path.join(file_path.split('/')[-3], file_path.split('/')[-2], file_path.split('/')[-1])  # 1CT009706 h\S0000000\00000186

                    mark_index_and_coordinates[img_number-j].append(RelativePath)
                    ...
                    # cv2.imshow('img', CT_img_solo)  #显示图像
                    # cv2.waitKey(1000)
                    # cv2.destroyAllWindows()
