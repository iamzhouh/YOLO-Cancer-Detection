...
# Data Loading
CSV_PATH = '../CTImageProcessing/label.csv'
DATA_PATH = '../MedicalDataSet'
...
img_paths = list(data_frame['imgPath'].array)
...
def path_to_image(path):
    ...
    image = pydicom.dcmread(os.path.join(DATA_PATH, path)).pixel_array
    return image
...
# print(list(imgs)[0].shape)

# print(np.array(list(imgs)).shape)
...
def log_loss(y_true, y_pred):
    ...
    loss = tf.negative(tf.compat.v1.log(iou))
    return loss
